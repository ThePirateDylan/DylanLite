...
def addon_themes():
	return [{'name': 'Light', 'value': ('FF3FA9FF', 'FFE9EEF5'), 'icon': 'light'}, {'name': 'Medium', 'value': ('FF3FA9FF', 'FF1A2330'), 'icon': 'medium'},
			{'name': 'Dark', 'value': ('FF3FA9FF', 'FF0F1420'), 'icon': 'dark'}]
...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.pirate').getAddonInfo(info)
...
def build_url(url_params):
	return 'plugin://plugin.video.pirate/?%s' % urlencode(url_params)
...
def addon(addon_id='plugin.video.pirate'):
...
def run_addon(addon='plugin.video.pirate', block=False):
...
def disable_enable_addon(addon_name='plugin.video.pirate'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.pirate'):
	import time
...
def notification(line1, time=5000, icon=None):
	kodi_dialog().notification('The Pirate Streaming', line1, icon or addon_icon(), time)
