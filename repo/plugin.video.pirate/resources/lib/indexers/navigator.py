...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get('name', 'The Pirate')
	...
	def changelog_utils(self):
		...
		fenlight_clogpath = k.translate_path('special://home/addons/plugin.video.pirate/resources/text/changelog.txt')
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				icon = iconImage
				if iconImage:
					if iconImage.startswith('http') or 'plugin.video.pirate' in iconImage: original_image = True
	...
	def tips(self):
		tips_location = 'special://home/addons/plugin.video.pirate/resources/text/tips'
